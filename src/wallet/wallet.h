...
{
public:
    libzeroinsight::PaymentAddress address;
    ...
    CNoteData(libzeroinsight::PaymentAddress a) :
            address {a}, nullifier(), witnessHeight {-1} { }
    CNoteData(libzeroinsight::PaymentAddress a, uint256 n) :
...
{
    JSOutPoint jsop;
    libzeroinsight::NotePlaintext plaintext;
};
...
public:
    ...
    std::map<libzeroinsight::PaymentAddress, CKeyMetadata> mapZKeyMetadata;
    ...
    bool AddZKey(const libzeroinsight::SpendingKey &key);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadZKey(const libzeroinsight::SpendingKey &key);
    //! Load spending key metadata (used by LoadWallet)
    bool LoadZKeyMetadata(const libzeroinsight::PaymentAddress &addr, const CKeyMetadata &meta);
    //! Adds an encrypted spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadCryptedZKey(const libzeroinsight::PaymentAddress &addr, const libzeroinsight::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);
    //! Adds an encrypted spending key to the store, and saves it to disk (virtual method, declared in crypter.h)
    bool AddCryptedSpendingKey(const libzeroinsight::PaymentAddress &address, const libzeroinsight::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);

    //! Adds a viewing key to the store, and saves it to disk.
    bool AddViewingKey(const libzeroinsight::ViewingKey &vk);
    bool RemoveViewingKey(const libzeroinsight::ViewingKey &vk);
    //! Adds a viewing key to the store, without saving it to disk (used by LoadWallet)
    bool LoadViewingKey(const libzeroinsight::ViewingKey &dest);
