        ...
        fromaddress_(fromAddress), t_outputs_(tOutputs), z_outputs_(zOutputs), mindepth_(minDepth), fee_(fee), contextinfo_(contextInfo)
{
    ...
    if (isfromzaddr_ && minDepth==0) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Minconf cannot be zeroinsight when sending from zaddr");
    }
...
bool AsyncRPCOperation_sendmany::main_impl() {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            auto hSig = prevJoinSplit.h_sig(*pzeroinsightParams, tx_.joinSplitPubKey);
            try {
        ...
        uint256 anchor)
{
            ...
            &esk); // parameter expects pointer to esk, so pass in address
    {
        ...
        if (!(jsdesc.Verify(*pzeroinsightParams, verifier, joinSplitPubKey_))) {
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pzeroinsightParams, joinSplitPubKey_);
    ...
    for (int i = 0; i < ZC_NUM_JS_OUTPUTS; i++) {
        ...
        libzeroinsight::PaymentAddress zaddr = output.addr;  // randomized output
