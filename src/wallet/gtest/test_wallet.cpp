...
CWalletTx GetValidReceive(const libzeroinsight::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libzeroinsight::Note GetNote(const libzeroinsight::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libzeroinsight::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libzeroinsight::SpendingKey::random();
    auto sk2 = libzeroinsight::SpendingKey::random();
