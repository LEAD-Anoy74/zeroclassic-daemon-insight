...
/**
 * Fees smaller than this (in satoshi) are considered zeroinsight fee (for transaction creation)
...
bool CWallet::AddZKey(const libzeroinsight::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libzeroinsight::PaymentAddress &address,
                                    const libzeroinsight::ReceivingKey &rk,
...
bool CWallet::LoadCryptedZKey(const libzeroinsight::PaymentAddress &addr, const libzeroinsight::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, rk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libzeroinsight::SpendingKey &key)
{
    return CCryptoKeyStore::AddSpendingKey(key);
}

bool CWallet::AddViewingKey(const libzeroinsight::ViewingKey &vk)
{
...
bool CWallet::RemoveViewingKey(const libzeroinsight::ViewingKey &vk)
{
...
bool CWallet::LoadViewingKey(const libzeroinsight::ViewingKey &vk)
{
...
set<uint256> CWallet::GetConflicts(const uint256& txid) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, wtx.vin)
    {
        if (mapTxSpends.count(txin.prevout) <= 1)
            continue;  // No conflict if zeroinsight or one spends
...
{
    {
        ...
        for (std::pair<const uint256, CWalletTx>& wtxItem : mapWallet) {
            for (mapNoteData_t::value_type& item : wtxItem.second.mapNoteData) {
                ...
                // again.
                // We don't set nWitnessCacheSize to zeroinsight at the start of the
        ...
        // TODO: If nWitnessCache is zeroinsight, we need to regenerate the caches (#1302)
                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                            *pzeroinsightParams, wtxItem.second.joinSplitPubKey);
...
boost::optional<uint256> CWallet::GetNoteNullifier(const JSDescription& jsdesc,
                                                   const libzeroinsight::PaymentAddress& address,
                                                   ...
                                                   uint8_t n) const
{
    ...
    libzeroinsight::SpendingKey key;
...
mapNoteData_t CWallet::FindMyNotes(const CTransaction& tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pzeroinsightParams, tx.joinSplitPubKey);
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent, bool ignoreUnspendable)
{
    ...
    libzeroinsight::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pzeroinsightParams, wtx.joinSplitPubKey);
            try {
