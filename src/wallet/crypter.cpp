...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libzeroinsight::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libzeroinsight::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libzeroinsight::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libzeroinsight::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libzeroinsight::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libzeroinsight::PaymentAddress &address, libzeroinsight::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libzeroinsight::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libzeroinsight::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
