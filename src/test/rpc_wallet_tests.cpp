...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_validateaddress)
{
    ...
    std::set<libzeroinsight::PaymentAddress> addrs;
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importwallet)
{
    ...
    auto testSpendingKey = libzeroinsight::SpendingKey::random();
    ...
    std::string format_str = "# Wallet dump created by Zeroinsight v0.11.2.0.z8-9155cc6-dirty (2016-08-11 11:37:00 -0700)\n"
...
BOOST_AUTO_TEST_CASE(rpc_z_sendmany_internals)
{
    ...
    // minconf cannot be zeroinsight when sending from zaddr
    {
        try {
            ...
            BOOST_CHECK(find_error(objError, "Minconf cannot be zeroinsight when sending from zaddr"));
        }
    }
    ...
    // get_memo_from_hex_string())
    {
        ...
        for (int i=4; i<ZC_MEMO_SIZE; i++) {
            BOOST_CHECK_EQUAL(array[i], 0x00);  // zeroinsight padding
        }
