...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    //
    // Also, it's generally libzeroinsight's job to ensure the
    // integrity of the scheme through its own tests.

    // construct a merkle tree
    ZCIncrementalMerkleTree merkleTree;

    libzeroinsight::SpendingKey k = libzeroinsight::SpendingKey::random();
    libzeroinsight::PaymentAddress addr = k.address();

    libzeroinsight::Note note(addr.a_pk, 100, uint256(), uint256());
    ...
    uint256 pubKeyHash;
    boost::array<libzeroinsight::JSInput, ZC_NUM_JS_INPUTS> inputs = {
        libzeroinsight::JSInput(witness, note, k),
        libzeroinsight::JSInput() // dummy input of zeroinsight value
    };
    boost::array<libzeroinsight::JSOutput, ZC_NUM_JS_OUTPUTS> outputs = {
        libzeroinsight::JSOutput(addr, 50),
        libzeroinsight::JSOutput(addr, 50)
    };
    ...
    {
        JSDescription jsdesc(*pzeroinsightParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pzeroinsightParams, verifier, pubKeyHash));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pzeroinsightParams, verifier, pubKeyHash));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(*pzeroinsightParams, pubKeyHash, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(*pzeroinsightParams, pubKeyHash, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(*pzeroinsightParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pzeroinsightParams, verifier, pubKeyHash));
    }
}
