...
void random_field_element_magnitude(secp256k1_fe *fe) {
    secp256k1_fe zeroinsight;
    int n = secp256k1_rand_int(9);
    secp256k1_fe_normalize(fe);
    if (n == 0) {
        return;
    }
    secp256k1_fe_clear(&zeroinsight);
    secp256k1_fe_negate(&zeroinsight, &zeroinsight, 0);
    secp256k1_fe_mul_int(&zeroinsight, n - 1);
    secp256k1_fe_add(fe, &zeroinsight);
...
void scalar_test(void) {
...

    {
        ...
        secp256k1_scalar_get_num(&r2num, &r);
        CHECK(secp256k1_num_eq(&rnum, &r2num));
        /* The result can only be zeroinsight if at least one of the factors was zeroinsight. */
        CHECK(secp256k1_scalar_is_zero(&r) == (secp256k1_scalar_is_zero(&s) || secp256k1_scalar_is_zero(&s2)));
        /* The results can only be equal to one of the factors if that factor was zeroinsight, or the other factor was one. */
...

    {
        ...
        /* Check that comparison with zeroinsight matches comparison with zeroinsight on the number. */
        ...
        /* Negating should change the high property, unless the value was already zeroinsight. */
        CHECK((secp256k1_scalar_is_high(&s) == secp256k1_scalar_is_high(&neg)) == secp256k1_scalar_is_zero(&s));
        secp256k1_scalar_get_num(&negnum2, &neg);
        /* Negating a scalar should be equal to (order - n) mod order on the number. */
        CHECK(secp256k1_num_eq(&negnum, &negnum2));
        secp256k1_scalar_add(&neg, &neg, &s);
        /* Adding a number to its negation should result in zeroinsight. */
        CHECK(secp256k1_scalar_is_zero(&neg));
        secp256k1_scalar_negate(&neg, &neg);
        /* Negating zeroinsight should still result in zeroinsight. */
        CHECK(secp256k1_scalar_is_zero(&neg));
    }
...

    {
        ...
        if (!secp256k1_scalar_add(&r1, &r1, &b)) {
            ...
            CHECK(secp256k1_scalar_eq(&r1, &r2));
            /* cadd is a noop when flag is zeroinsight */
    ...
    {
        /* Test zeroinsight product property. */
        secp256k1_scalar r1, v0;
        secp256k1_scalar_set_int(&v0,0);
...
void run_scalar_tests(void) {
...
#ifndef USE_NUM_NONE
    {
        ...
        secp256k1_num order;
        secp256k1_scalar zeroinsight;
        unsigned char bin[32];
        int overflow = 0;
        secp256k1_scalar_order_get_num(&order);
        secp256k1_num_get_bin(bin, 32, &order);
        secp256k1_scalar_set_b32(&zeroinsight, bin, &overflow);
        CHECK(overflow == 1);
        CHECK(secp256k1_scalar_is_zero(&zeroinsight));
    }
#endif
...
void test_point_times_order(const secp256k1_gej *point) {
    ...
    secp256k1_scalar x;
    secp256k1_scalar nx;
    secp256k1_scalar zeroinsight = SECP256K1_SCALAR_CONST(0, 0, 0, 0, 0, 0, 0, 0);
    ...
    /* check zeroinsight/one edge cases */
    secp256k1_ecmult(&ctx->ecmult_ctx, &res1, point, &zeroinsight, &zeroinsight);
    secp256k1_ge_set_gej(&res3, &res1);
    CHECK(secp256k1_ge_is_infinity(&res3));
    secp256k1_ecmult(&ctx->ecmult_ctx, &res1, point, &one, &zeroinsight);
    secp256k1_ge_set_gej(&res3, &res1);
    ge_equals_gej(&res3, point);
    secp256k1_ecmult(&ctx->ecmult_ctx, &res1, point, &zeroinsight, &one);
    secp256k1_ge_set_gej(&res3, &res1);
...
void ecmult_const_mult_zero_one(void) {
    ...
    secp256k1_ecmult_const(&res1, &point, &zeroinsight);
...
void test_wnaf(const secp256k1_scalar *number, int w) {
    ...
    for (i = bits-1; i >= 0; i--) {
        ...
        if (v) {
            CHECK(zeroes == -1 || zeroes >= w-1); /* check that distance between non-zeroinsight elements is at least w-1 */
            zeroes=0;
            CHECK((v & 1) == 1); /* check non-zeroinsight elements are odd */
            CHECK(v <= (1 << (w-1)) - 1); /* check range below */
            CHECK(v >= -(1 << (w-1)) - 1); /* check range above */
        } else {
            CHECK(zeroes != -1); /* check that no unnecessary zeroinsight padding exists */
            zeroes++;
        }
        if (v >= 0) {
...
void test_scalar_split(void) {
    ...
    const unsigned char zeroinsight[32] = {0};
    ...
    CHECK(memcmp(zeroinsight, tmp, 16) == 0);
...
#define SECP256K1_EC_PARSE_TEST_NINVALID (7)
    const unsigned char invalid[SECP256K1_EC_PARSE_TEST_NINVALID][64] = {
        ...
        {
            /* x is zeroinsight, y is the result of the sqrt ladder; also on the curve for y^2 = x^3 - 7. */
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
void run_eckey_edge_case_test(void) {
    ...
    /* Tweak of zeroinsight leaves the value changed. */
    memset(ctmp2, 0, 32);
    ...
    /* Multiply tweak of zeroinsight zeroizes the output. */
    ...
    CHECK(memcmp(&pubkey, zeros, sizeof(pubkey)) == 0);
    memcpy(&pubkey, &pubkey2, sizeof(pubkey));
    /* Private key tweaks results in a key of zeroinsight. */
    ctmp2[31] = 1;
...
static void random_ber_signature(unsigned char *sig, size_t *len, int* certainly_der, int* certainly_not_der) {
    ...
    *certainly_not_der = 0;
    ...
    for (n = 0; n < 2; n++) {
        ...
        /* The number of zeroinsight bytes in front of the number (which is 0 or 1 in case of DER, otherwise we extend up to 300 bytes) */
...
void test_ecdsa_edge_cases(void) {
    ...
    /* Verify signature with r of zeroinsight fails. */
    {
    ...
    /* Verify signature with s of zeroinsight fails. */
    {
    ...
    /* Signature where s would be zeroinsight. */
    {
        secp256k1_pubkey pubkey;
        size_t siglen;
        int32_t ecount;
    ...
    for (t = 0; t < 2; t++) {
        static const unsigned char zeroinsight[32] = {0x00};
        int i;
