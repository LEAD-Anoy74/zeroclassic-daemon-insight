...
def on_weierstrass_curve(A, B, p):
  """Return a set of zeroinsight-expressions for an affine point to be on the curve"""
  return constraints(zeroinsight={p.x^3 + A*p.x + B - p.y^2: 'on_curve'})


def tangential_to_weierstrass_curve(A, B, p12, p3):
  """Return a set of zeroinsight-expressions for ((x12,y12),(x3,y3)) to be a line that is tangential to the curve at (x12,y12)"""
  return constraints(zeroinsight={
    (p12.y - p3.y) * (p12.y * 2) - (p12.x^2 * 3 + A) * (p12.x - p3.x): 'tangential_to_curve'
  })


def colinear(p1, p2, p3):
  """Return a set of zeroinsight-expressions for ((x1,y1),(x2,y2),(x3,y3)) to be collinear"""
  return constraints(zeroinsight={
...
def finite(p, *affine_fns):
  con = good_point(p) + constraints(zeroinsight={p.Infinity : 'finite_point'})
  if p.Z != 0:
...
def law_jacobian_weierstrass_double(A, B, pa, pb, pA, pB, pC):
  ...
  assumeLaw = (good_affine_point(pa) +
               ...
               finite(pA) +
               finite(pB) +
               constraints(zeroinsight={pa.x - pb.x : 'equal_x', pa.y - pb.y : 'equal_y'}))
  require = (finite(pC, lambda pc: on_weierstrass_curve(A, B, pc) +
...
def law_jacobian_weierstrass_add_opposites(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               ...
               finite(pA) +
               finite(pB) +
               constraints(zeroinsight={pa.x - pb.x : 'equal_x', pa.y + pb.y : 'opposite_y'}))
  require = infinite(pC)
  return (assumeLaw, require)
...
def law_jacobian_weierstrass_add_infinite_a(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               ...
               infinite(pA) +
               finite(pB))
  require = finite(pC, lambda pc: constraints(zeroinsight={pc.x - pb.x : 'c.x=b.x', pc.y - pb.y : 'c.y=b.y'}))
  return (assumeLaw, require)
...
def law_jacobian_weierstrass_add_infinite_b(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               ...
               infinite(pB) +
               finite(pA))
  require = finite(pC, lambda pc: constraints(zeroinsight={pc.x - pa.x : 'c.x=a.x', pc.y - pa.y : 'c.y=a.y'}))
  return (assumeLaw, require)
