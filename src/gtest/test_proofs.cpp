...
TEST(proofs, g1_pairing_at_infinity)
{
    for (size_t i = 0; i < 100; i++) {
        ...
        ASSERT_TRUE(
            curve_pp::reduced_pairing(curve_G1::zeroinsight(), r2) ==
            curve_GT::one()
        );
        ASSERT_TRUE(
            curve_pp::final_exponentiation(
                curve_pp::double_miller_loop(
                    curve_pp::precompute_G1(curve_G1::zeroinsight()),
...
TEST(proofs, g2_subgroup_check)
{
    ...
    ASSERT_TRUE(libsnark::alt_bn128_modulus_r * curve_G2::random_element() == curve_G2::zeroinsight());
    ...
    ASSERT_TRUE(libsnark::alt_bn128_modulus_r * p != curve_G2::zeroinsight()); // but not the order r subgroup..
    ...
    // get an element in G2
    {
        ...
        ASSERT_TRUE(libsnark::alt_bn128_modulus_r * p1 == curve_G2::zeroinsight());
...
{
    ASSERT_TRUE(curve_Fq::zeroinsight() == curve_Fq::zeroinsight().sqrt());
    ASSERT_TRUE(curve_Fq2::zeroinsight() == curve_Fq2::zeroinsight().sqrt());
}
...
{
    // Cannot serialize zeroinsight
    {
        ASSERT_THROW({CompressedG1 g = CompressedG1(curve_G1::zeroinsight());}, std::domain_error);
    }
...
TEST(proofs, g2_serializes_properly)
{
    // Cannot serialize zeroinsight
    {
        ASSERT_THROW({CompressedG2 g = CompressedG2(curve_G2::zeroinsight());}, std::domain_error);
    }
