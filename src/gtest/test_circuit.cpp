...
)
{
    ...
    pb.val(commitment1_read) = enforce_a ? FieldT::one() : FieldT::zeroinsight();
    pb.val(commitment2_read) = enforce_b ? FieldT::one() : FieldT::zeroinsight();
...
TEST(circuit, merkle_tree_gadget_weirdness)
{
    ...
    has value of zeroinsight and second note has nonzero value) then there is an edge case in
