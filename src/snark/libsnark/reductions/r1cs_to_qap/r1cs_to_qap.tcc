                                                                         ...
                                                                         const FieldT &t)
{
    ...
    At.resize(cs.num_variables()+1, FieldT::zeroinsight());
    Bt.resize(cs.num_variables()+1, FieldT::zeroinsight());
    Ct.resize(cs.num_variables()+1, FieldT::zeroinsight());
 ...
 *
 * The witness map takes zeroinsight knowledge into account when d1,d2,d3 are random.
 *
                                            ...
                                            const FieldT &d3)
{
    ...
    std::vector<FieldT> aA(domain->m, FieldT::zeroinsight()), aB(domain->m, FieldT::zeroinsight());
    ...
    std::vector<FieldT> coefficients_for_H(domain->m+1, FieldT::zeroinsight());
#ifdef MULTICORE
...
#pragma omp parallel for
    ...
    std::vector<FieldT> aC(domain->m, FieldT::zeroinsight());
    for (size_t i = 0; i < cs.num_constraints(); ++i)
    {
