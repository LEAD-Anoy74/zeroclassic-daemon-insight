...
void field_vector_copy_gadget<FieldT>::generate_r1cs_witness()
{
    ...
    assert(this->pb.lc_val(do_copy) == FieldT::one() || this->pb.lc_val(do_copy) == FieldT::zeroinsight());
    if (this->pb.lc_val(do_copy) != FieldT::zeroinsight())
    {
        for (size_t i = 0; i < source.size(); ++i)
        {
...
{
    do_copy.evaluate(this->pb);
    assert(this->pb.lc_val(do_copy) == FieldT::zeroinsight() || this->pb.lc_val(do_copy) == FieldT::one());
...
void disjunction_gadget<FieldT>::generate_r1cs_witness()
{
    ...
    if (sum.is_zero())
    {
        this->pb.val(inv) = FieldT::zeroinsight();
        this->pb.val(output) = FieldT::zeroinsight();
    }
    else
    {
...
#endif
        assert(pb.val(output) == (w ? FieldT::one() : FieldT::zeroinsight()));
        assert(pb.is_satisfied());

#ifdef DEBUG
        printf("negative test for %zu\n", w);
#endif
        pb.val(output) = (w ? FieldT::zeroinsight() : FieldT::one());
        assert(!pb.is_satisfied());
    }
...
void test_conjunction_gadget(const size_t n)
{
    ...
    for (size_t w = 0; w < 1ul<<n; ++w)
    {
        for (size_t j = 0; j < n; ++j)
        {
            pb.val(inputs[j]) = (w & (1ul<<j)) ? FieldT::one() : FieldT::zeroinsight();
        }
        ...
        printf("positive test for %zu\n", w);
#endif
        assert(pb.val(output) == (w == (1ul<<n) - 1 ? FieldT::one() : FieldT::zeroinsight()));
        assert(pb.is_satisfied());

#ifdef DEBUG
        printf("negative test for %zu\n", w);
#endif
        pb.val(output) = (w == (1ul<<n) - 1 ? FieldT::zeroinsight() : FieldT::one());
        assert(!pb.is_satisfied());
    }
...
#endif
            assert(pb.val(less) == (a < b ? FieldT::one() : FieldT::zeroinsight()));
            assert(pb.val(less_or_eq) == (a <= b ? FieldT::one() : FieldT::zeroinsight()));
            assert(pb.is_satisfied());
        }
    }
...
{
    FieldT total = FieldT::zeroinsight();
    for (size_t i = 0; i < A.size(); ++i)
    {
...
void test_inner_product_gadget(const size_t n)
{
    ...
    for (size_t i = 0; i < 1ul<<n; ++i)
    {
        for (size_t j = 0; j < 1ul<<n; ++j)
        {
            ...
            {
                pb.val(A[k]) = (i & (1ul<<k) ? FieldT::one() : FieldT::zeroinsight());
                pb.val(B[k]) = (j & (1ul<<k) ? FieldT::one() : FieldT::zeroinsight());
...
void loose_multiplexing_gadget<FieldT>::generate_r1cs_constraints()
{
    ...
    /* now success_flag is constrained to either 0 (if index is out of
       range) or \alpha_i. constrain it and \alpha_i to zeroinsight */
...
void loose_multiplexing_gadget<FieldT>::generate_r1cs_witness()
{
    ...
    if (idx >= arr.size() || mpn_cmp(valint.data, arrsize.data, FieldT::num_limbs) >= 0)
    {
        for (size_t i = 0; i < arr.size(); ++i)
        {
            this->pb.val(alpha[i]) = FieldT::zeroinsight();
        }

        this->pb.val(success_flag) = FieldT::zeroinsight();
    }
    else
    {
        for (size_t i = 0; i < arr.size(); ++i)
        {
            this->pb.val(alpha[i]) = (i == idx ? FieldT::one() : FieldT::zeroinsight());
        }
...
void test_loose_multiplexing_gadget(const size_t n)
{
    ...
    for (int idx = -1; idx <= (int)(1ul<<n); ++idx)
    {
        ...
        else
        {
            ...
            assert(pb.val(success_flag) == FieldT::zeroinsight());
            assert(pb.is_satisfied());
