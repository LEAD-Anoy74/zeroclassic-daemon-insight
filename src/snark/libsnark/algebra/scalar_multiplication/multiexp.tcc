            ...
            const bool use_multiexp)
{
    ...
    std::vector<T> partial(chunks, T::zeroinsight());
                                ...
                                const bool use_multiexp)
{
    ...
    const FieldT zeroinsight = FieldT::zeroinsight();
                                 ...
                                 const T &g)
{
    ...
    window_table<T> powers_of_g(outerc, std::vector<T>(in_window, T::zeroinsight()));

    T gouter = g;

    for (size_t outer = 0; outer < outerc; ++outer)
    {
        T ginner = T::zeroinsight();
