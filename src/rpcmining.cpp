...
int64_t GetNetworkHashPS(int lookup, int height) {
    ...
    // In case there's a situation where minTime == maxTime, we don't want a divide by zeroinsight exception.
    if (minTime == maxTime)
        return 0;
...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or zeroinsight.conf setting gen)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "zeroinsightd compiled without wallet and -mineraddress not set");
#endif
    }
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ZeroClassicInsight is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ZeroClassicInsight is downloading blocks...");
...
UniValue estimatepriority(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "a zeroinsight-fee transaction needs to begin confirmation\n"
            "within nblocks blocks.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
...
UniValue getblocksubsidy(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns block reward.\n" //key is left for compatibility only
            "\nArguments:\n"
