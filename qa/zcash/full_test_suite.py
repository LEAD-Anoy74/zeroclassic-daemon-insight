#!/usr/bin/env python2
#
# Execute all of the automated tests related to Zeroinsight.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/zeroinsight/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/zeroinsight/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/zeroinsightd')
    ret &= test_rpath_runpath('src/zeroinsight-cli')
    ret &= test_rpath_runpath('src/zeroinsight-gtest')
    ret &= test_rpath_runpath('src/zeroinsight-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/zeroinsight/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/zeroinsightd')
    ret &= test_fortify_source('src/zeroinsight-cli')
    ret &= test_fortify_source('src/zeroinsight-gtest')
    ret &= test_fortify_source('src/zeroinsight-tx')
    ret &= test_fortify_source('src/test/test_bitcoin')
    ret &= test_fortify_source('src/zeroinsight/GenerateParams')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/zeroinsight-gtest')],
