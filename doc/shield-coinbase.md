...
The current Zeroinsight protocol includes a consensus rule that coinbase rewards must be sent to a shielded address.
...
Sweep up coinbase UTXOs from a transparent address you use for mining:
    ...
    zeroinsight-cli z_shieldcoinbase tMyMiningAddress zMyPrivateAddress

Sweep up coinbase UTXOs from multiple transparent addresses to a shielded address:

    zeroinsight-cli z_shieldcoinbase "*" zMyPrivateAddress

Sweep up with a fee of 1.23 ZEC:

    zeroinsight-cli z_shieldcoinbase tMyMiningAddress zMyPrivateAddress 1.23

Sweep up with a fee of 0.1 ZEC and set limit on the maximum number of UTXOs to shield at 25:

    zeroinsight-cli z_shieldcoinbase "*" zMyPrivateAddress 0.1 25
...
If the limit parameter is set to zeroinsight, the zeroinsightd `mempooltxinputlimit` option will be used instead, where the default value for `mempooltxinputlimit` is zeroinsight, which means no limit.
...
If no limit is set (in the case when limit parameter and `mempooltxinputlimit` options are set to zeroinsight) the behaviour of z_shieldcoinbase is to consume as many UTXOs as possible, with `z_shieldcoinbase` constructing a transaction up to the size limit of 100,000 bytes.
